...
This means requesting `locale: 'fr'` today gives you a real mix — translated strings where `fr.json` has the key, English strings where it doesn't — never a blank, `undefined`, or thrown error. See `t()` in `scripts/build-core.js` if you need the exact implementation.
...
| `reason` | Meaning |
|---|---|
| `ok` | The requested dictionary was used and carries every key `en.json` has. |
