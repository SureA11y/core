...
test('contrast perf: computeEffectiveBackground caches per element+opts when collectStack=false', () => {
  ...
  assert.strictEqual(
    out2,
    out1,
    'cached result should be same reference for identical opts when collectStack=false'
  );
});
...
test('contrast perf: computeEffectiveBackground does not memoize results when collectStack=true', () => {
  ...
  assert.notStrictEqual(
    out2,
    out1,
    'should not reuse the same cached object reference when collectStack=true'
  );
  ...
  assert.notStrictEqual(
    out2.stack,
    out1.stack,
    'stack array should be newly allocated each call when collectStack=true'
  );
