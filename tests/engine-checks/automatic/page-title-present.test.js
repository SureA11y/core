...
test('pass when <title> is non-empty and inside <head>', () => {
    const html = '<!doctype html><html lang="en"><head><title>Acme Contact Support</title></head><body>Hi</body></html>';
...
test(`${RULE_ID}: fixture coverage (tests/fixtures/page-title-present-all-scenarios.html)`, () => {
    const fixturePath = path.join(__dirname, '../..', 'fixtures', 'page-title-present-all-scenarios.html');
...
test('pass when <title> is non-empty but ends up outside <head> (in <body>)', () => {
    ...
    const html = '<!doctype html><html lang="en"><head><meta charset="utf-8"></head><body><title>Acme Contact Support</title>Hi</body></html>';
