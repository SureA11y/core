...
test(`${RULE_ID}: notApplicable when no element has aria-errormessage`, () => {
  const html = `<!doctype html><html><body><input aria-invalid="true"></body></html>`;
  const result = runa11yCoreOnHtml(html, { runOnly: [RULE_ID] });
  assertRule(result, RULE_ID, 'notApplicable', { minOccurrences: 0, maxOccurrences: 0 });
});

test(`${RULE_ID}: pass when aria-invalid="true"`, () => {
  const html = `<!doctype html><html><body><input aria-errormessage="e" aria-invalid="true"><div id="e">Error</div></body></html>`;
  const result = runa11yCoreOnHtml(html, { runOnly: [RULE_ID] });
  assertRule(result, RULE_ID, 'pass', { minOccurrences: 0, maxOccurrences: 0 });
});

test(`${RULE_ID}: cantTell when aria-invalid is absent`, () => {
...
test(`${RULE_ID}: cantTell when aria-invalid="false"`, () => {
  const html = `<!doctype html><html><body><input aria-errormessage="e" aria-invalid="false"><div id="e">Error</div></body></html>`;
  const result = runa11yCoreOnHtml(html, { runOnly: [RULE_ID] });
...
test(`${RULE_ID}: fixture coverage (tests/fixtures/aria-conditional-attr-all-scenarios.html)`, () => {
  ...
  const expectedFailIds = ['aca_case_04', 'aca_case_05'];
  const expectedNoOccIds = ['aca_case_01', 'aca_case_02', 'aca_case_03', 'aca_case_06'];
