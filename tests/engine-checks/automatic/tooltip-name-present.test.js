...
const RULE_ID = 'a11ycore-tooltip-name-present';
...
test(`${RULE_ID}: pass when tooltip has visible text content`, () => {
  const html = `<!doctype html><html><body><div role="tooltip">Helpful hint</div></body></html>`;
  const result = runa11yCoreOnHtml(html, { runOnly: [RULE_ID] });
  assertRule(result, RULE_ID, 'pass', { minOccurrences: 0, maxOccurrences: 0 });
});

test(`${RULE_ID}: fail when tooltip has no accessible name`, () => {
  const html = `<!doctype html><html><body><div id="a" role="tooltip"></div></body></html>`;
  const result = runa11yCoreOnHtml(html, { runOnly: [RULE_ID] });
  const rule = assertRule(result, RULE_ID, 'fail', { minOccurrences: 1, maxOccurrences: 1 });
  assert.equal(rule.occurrences[0].data.details.controlType, 'tooltip');
});
...
test(`${RULE_ID}: i18n default is English`, () => {
  ...
  assert.strictEqual(rule.title, 'Tooltips have an accessible name');
});

test(`${RULE_ID}: pass when the tooltip's name comes from a wrapped img alt (name-from-content recursion)`, () => {
  const html = `<!doctype html><html><body><div role="tooltip" id="tt1"><img alt="Helpful hint" src="x.png"></div></body></html>`;
  const result = runa11yCoreOnHtml(html, { runOnly: [RULE_ID] });
  assertRule(result, RULE_ID, 'pass', { minOccurrences: 0, maxOccurrences: 0 });
});
...
test(`${RULE_ID}: aria-labelledby pointing at an <iframe> falls back to its title attribute => pass`, () => {
  ...
  const html = `<!doctype html><html><body><iframe id="t" title="Settings"></iframe><div role="tooltip" aria-labelledby="t"></div></body></html>`;
  const result = runa11yCoreOnHtml(html, { runOnly: [RULE_ID] });
  assertRule(result, RULE_ID, 'pass', { minOccurrences: 0, maxOccurrences: 0 });
});