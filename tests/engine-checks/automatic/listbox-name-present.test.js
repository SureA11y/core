...
test('listbox-name-present: wrapping <label> has its own aria-label even though its only child content is aria-hidden', () => {
...
test(`${RULE_ID}: fixture coverage (tests/fixtures/listbox-name-present-all-scenarios.html)`, () => {
  const fixturePath = path.join(
    ...
    '../..',
    'fixtures',
    'listbox-name-present-all-scenarios.html'
  );
...
test("listbox-name-present: label association with empty content falls back to the label's own title attribute => pass", () => {
  ...
  // Uses a native, genuinely labelable <input>, not role="listbox" on a
  ...
  // labelled at all.
  const html = `<!doctype html><html><body><label for='a' title='Search'></label><input id='a' role='listbox'></body></html>`;
