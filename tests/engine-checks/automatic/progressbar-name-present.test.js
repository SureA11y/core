...
test(`${RULE_ID}: notApplicable when no role="progressbar" is present`, () => {
...
test(`${RULE_ID}: fail when progressbar has no accessible name`, () => {
  const html = `<!doctype html><html><body><div id="a" role="progressbar" aria-valuenow="50"></div></body></html>`;
  const result = runa11yCoreOnHtml(html, { runOnly: [RULE_ID] });
  const rule = assertRule(result, RULE_ID, 'fail', { minOccurrences: 1, maxOccurrences: 1 });
  assert.equal(rule.occurrences[0].data.details.controlType, 'progressbar');
});
...
test(`${RULE_ID}: fixture coverage (tests/fixtures/progressbar-name-present-all-scenarios.html)`, () => {
  const fixturePath = path.join(__dirname, '../..', 'fixtures', 'progressbar-name-present-all-scenarios.html');
...
test(`${RULE_ID}: aria-labelledby pointing at an <iframe> falls back to its title attribute => pass`, () => {
  ...
  const html = `<!doctype html><html><body><iframe id="t" title="Settings"></iframe><div role="progressbar" aria-labelledby="t"></div></body></html>`;
  const result = runa11yCoreOnHtml(html, { runOnly: [RULE_ID] });
