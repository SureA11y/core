...
const RULE_ID = 'a11ycore-no-autoplay-audio';
...
test(`${RULE_ID}: i18n default is English`, () => {
  ...
  assert.strictEqual(rule.title, 'Autoplaying audio should provide a pause/stop or volume-control mechanism');
});

test(`${RULE_ID}: fixture coverage (tests/fixtures/no-autoplay-audio-all-scenarios.html)`, () => {
  const fixturePath = path.join(__dirname, '../..', 'fixtures', 'no-autoplay-audio-all-scenarios.html');
