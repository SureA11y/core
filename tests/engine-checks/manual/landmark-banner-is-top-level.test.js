...
test(`${RULE_ID}: notApplicable when a <header> is nested inside <main> (loses implicit banner role entirely)`, () => {
  const html = `<!doctype html><html><body><main><header>inner</header></main></body></html>`;
  const result = runa11yCoreOnHtml(html, { runOnly: [RULE_ID] });
  assertRule(result, RULE_ID, 'notApplicable', { minOccurrences: 0, maxOccurrences: 0 });
});
...
test(`${RULE_ID}: fixture coverage (tests/fixtures/landmark-banner-is-top-level-all-scenarios.html)`, () => {
  const fixturePath = path.join(__dirname, '../..', 'fixtures', 'landmark-banner-is-top-level-all-scenarios.html');
