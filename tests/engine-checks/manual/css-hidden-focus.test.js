...
test(`${RULE_ID}: cantTell when multiple visibility hints apply (opacity:0 + offscreen)`, () => {
    ...
    const result = runa11yCoreOnHtml(html, { runOnly: [RULE_ID] });
    const rule = assertRule(result, RULE_ID, 'cantTell', { minOccurrences: 1, maxOccurrences: 1 });
    assert.strictEqual(rule.occurrences[0].data.details.metrics.visibilityHints.join(','), 'opacityZero,offscreen');
});
...
test(`${RULE_ID}: fixture coverage (tests/fixtures/css-hidden-focus-all-scenarios.html)`, () => {
    const fixturePath = path.join(__dirname, '../..', 'fixtures', 'css-hidden-focus-all-scenarios.html');
    ...
    for (const [id, hints] of Object.entries(expectedHintsById)) {
        const occ = rule.occurrences.find((o) => typeof o.html === 'string' && o.html.includes(`id="${id}"`));
        assert.ok(occ, `Expected occurrence for id="${id}"`);
        assert.deepStrictEqual(occ.data.details.metrics.visibilityHints, hints, `Expected hints for id="${id}"`);
    }
