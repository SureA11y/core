...
test('notApplicable (not pass) for a normal, descriptive title', () => {
  ...
  assert.notStrictEqual(
    rule.outcome,
    'pass',
    'manual rule must never report pass, even for a good title'
  );
});
...
test('cantTell with reasonCode duplicateTitlesAcrossPages when >=10 pages share the same title', () => {
  ...
  const probes = {
    'crawl.pageTitles': { pages: pagesOf(10, { duplicateTitle: 'Acme Corporation' }) }
  };
...
test('cantTell with reasonCode templatedTitlesAcrossPages when titles share a long common prefix/suffix', () => {
  ...
  const probes = {
    'crawl.pageTitles': { pages: pagesOf(10, { templatePrefix: 'Acme Corporation' }) }
  };
...
test('falls back to single-page heuristics when fewer than 10 analyzable pages are provided', () => {
  ...
  const html = withTitle('Home');
  const probes = {
    'crawl.pageTitles': { pages: pagesOf(5, { duplicateTitle: 'Acme Corporation' }) }
  };
...
test(`${RULE_ID}: runDomRulesInPage (Node/require entry point) agrees with runa11yCoreInPage across the cross-page probe branches`, () => {
  ...
  const scenarios = [
    {
      html: withTitle('Acme Corporation'),
      probes: { 'crawl.pageTitles': { pages: pagesOf(10, { duplicateTitle: 'Acme Corporation' }) } }
    },
    {
      html: withTitle('Acme Corporation - Section 0'),
      probes: { 'crawl.pageTitles': { pages: pagesOf(10, { templatePrefix: 'Acme Corporation' }) } }
    },
    {
      html: withTitle('Distinct Fully Unique Title'),
      probes: { 'crawl.pageTitles': { pages: pagesOf(10) } }
    },
    {
      html: withTitle('Home'),
      probes: { 'crawl.pageTitles': { pages: pagesOf(5, { duplicateTitle: 'Acme Corporation' }) } }
    }
  ];
...
test(`${RULE_ID}: fixture coverage (tests/fixtures/page-title-patterns-all-scenarios.html)`, () => {
  const fixturePath = path.join(
    ...
    '../..',
    'fixtures',
    'page-title-patterns-all-scenarios.html'
  );
...
test(`page-title-patterns: notApplicable when contextSelector scopes narrower than the whole document (fragment-scan applicability)`, () => {
  ...
  const result = runa11yCoreOnHtml(html, {
    runOnly: ['page-title-patterns'],
    contextSelector: 'body'
  });
  assertRule(result, 'page-title-patterns', 'notApplicable', {
    minOccurrences: 0,
    maxOccurrences: 0
  });
});
