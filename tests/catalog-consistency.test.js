...
test('composite catalog references only known atomic checks', () => {
  ...
  for (const c of composites) {
    ...
    assert.ok(
      Array.isArray(c.checksIds) && c.checksIds.length > 0,
      `Composite ${c.id} must have checksIds`
    );
...
test('every WCAG-mapped atomic test is covered by at least one composite for that SC', () => {
  assert.ok(
    Array.isArray(core.CHECK_DEFS),
    'Expected core.CHECK_DEFS to exist (generated core export)'
  );
  assert.ok(
    Array.isArray(core.COMPOSITE_RULES),
    'Expected core.COMPOSITE_RULES to exist (generated core export)'
  );
  ...
  for (const r of core.CHECK_DEFS) {
    ...
    for (const sc of scs) {
      ...
      assert.ok(
        set.has(r.ruleId),
        `Atomic test ${r.ruleId} (SC ${sc}) is not included in any composite for that SC`
      );
    }
  }
});
...
test('WCAG composite membership is exact for every WCAG-mapped atomic test (no missing, no extra)', () => {
  ...
  for (const c of core.COMPOSITE_RULES) {
    ...
    assert.equal(
      scList.length,
      1,
      `Composite ${c && c.id ? c.id : '(unknown)'} must declare exactly one wcagSc`
    );
    const sc = scList[0];
  ...
  for (const r of core.CHECK_DEFS) {
    ...
    if (declaredScs.size === 0) {
      ...
      assert.ok(
        !inComposites || inComposites.size === 0,
        `Atomic test ${testId} appears in composites but declares no WCAG normativeMappings`
      );
      continue;
    }
    ...
    for (const sc of declaredScs) {
      ...
      assert.ok(
        set.has(testId),
        `Atomic test ${testId} declares SC ${sc} but is not included in any composite for that SC`
      );
    }
...
test('contract: rule.wcagSc is derived from WCAG normativeMappings (exact match)', () => {
  ...
  for (const r of core.CHECK_DEFS) {
    ...
    const same = fromNorm.length === fromRule.length && fromNorm.every((v, i) => v === fromRule[i]);
