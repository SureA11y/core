...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === '--strictFacets') args.strictFacets = true;
  }
  return args;
}
...
function resolveFacetsFile(repoRoot, facetsArg) {
  if (facetsArg) {
    return path.isAbsolute(facetsArg) ? facetsArg : path.resolve(repoRoot, facetsArg);
  }
...
/**
 * Extract WCAG SC identifiers from rule meta.
 * - Normative SCs come from normativeMappings where standard === 'WCAG'
 * - Informative SCs come from informativeReferences where standard === 'WCAG' and type === 'SC'
 *
 * Returns { normativeScs: string[], informativeScs: string[], allScs: string[] }
 */
function extractWcagScFromMeta(meta) {
  const normative = [];
  const informative = [];

  const nm = normalizeObjectArray(meta && meta.normativeMappings);
  for (const m of nm) {
    const std = String(m.standard || '')
      .trim()
      .toUpperCase();
    if (!(std === 'WCAG' || std.startsWith('WCAG '))) continue;
    if (!m.requirement) continue;
    normative.push(normalizeSc(m.requirement));
  }

  const ir = normalizeObjectArray(meta && meta.informativeReferences);
  for (const r of ir) {
    if (String(r.standard || '').toUpperCase() !== 'WCAG') continue;
    if (String(r.type || '').toUpperCase() !== 'SC') continue;
    if (!r.requirement) continue;
    informative.push(normalizeSc(r.requirement));
  }

  const norm = uniqueStrings(normative).filter(Boolean);
  const info = uniqueStrings(informative).filter(Boolean);

  // stable union (normative first)
  const all = uniqueStrings([...norm, ...info]).filter(Boolean);

  return { wcagScNormative: norm, wcagScInformative: info, wcagScAll: all };
}
...
function getMissingFacetRulesForSc(sc, rowsForSc) {
  ...
  for (const r of rowsForSc) {
    ...
    // For SC sections only (not "(unmapped)"), a row here implies extracted WCAG SC coverage includes sc (from normativeMappings and/or informativeReferences)
...
function summarizeFacetCoverage(sc, facetDefs, rowsForSc) {
  ...
  for (const r of rowsForSc) {
    ...
    for (const facetId of facetIds) {
      ...
      if (!facet) {
        if (!unknownFacetIds.includes(facetId)) unknownFacetIds.push(facetId);
        continue;
      }
...
function extractWcagSignalsFromTags(tags) {
  ...
  for (const v of versions)
    cumulativeLevelsByVersion[v] = Array.from(byVersion.get(v).values()).sort();
...
function main() {
  ...
  for (const file of files) {
    ...
    const scExtract = extractWcagScFromMeta(meta);
    const scsNormative = scExtract.wcagScNormative;
    const scsAll = scExtract.wcagScAll;
    ...
    rows.push({
      file: path.relative(repoRoot, file),
      ruleId,
      wcagScNormative: scsNormative,
      wcagScAll: scsAll,
      wcagScInformative: scExtract.wcagScInformative,
  ...
  // Group by SC
  function groupBySc(rows, key) {
    const bySc = new Map();
    for (const r of rows) {
      const list = r && Array.isArray(r[key]) ? r[key] : [];
      const scList = list.length ? list : ['(unmapped)'];
  ...
  // A: enforced coverage only
  const byScNormative = groupBySc(rows, 'wcagScNormative');

  // B: enforced + informative/manual coverage
  const byScAll = groupBySc(rows, 'wcagScAll');
  ...
  md += renderCoverageTableMarkdown('Coverage by WCAG Level (Per version, cumulative)', [
    {
      label: 'WCAG 2.0',
      A: coverageCounters.byVersion['2.0'].A || 0,
      AA: coverageCounters.byVersion['2.0'].AA || 0,
      AAA: coverageCounters.byVersion['2.0'].AAA || 0
    },
    {
      label: 'WCAG 2.1',
      A: coverageCounters.byVersion['2.1'].A || 0,
      AA: coverageCounters.byVersion['2.1'].AA || 0,
      AAA: coverageCounters.byVersion['2.1'].AAA || 0
    },
    {
      label: 'WCAG 2.2',
      A: coverageCounters.byVersion['2.2'].A || 0,
      AA: coverageCounters.byVersion['2.2'].AA || 0,
      AAA: coverageCounters.byVersion['2.2'].AAA || 0
    }
  ]);

  // Tag totals (raw) for transparency/debugging
  const rawTags = Object.keys(coverageCounters.rawTagCounts || {}).sort((a, b) =>
  ...
  function countUnknownFacetIds(list) {
    let n = 0;
    for (const s of Array.isArray(list) ? list : []) {
      const u = s && Array.isArray(s.unknownFacetIds) ? s.unknownFacetIds : [];
      if (u.length) n += u.length;
    }
    return n;
  }
  ...
  function renderScSections(sectionTitle, byScMap, facetSink) {
    let mdSection = `## ${sectionTitle}\n\n`;

    const scKeys = Array.from(byScMap.keys()).sort((a, b) => {
    ...
    for (const sc of scKeys) {
      const rowsForSc = byScMap.get(sc) || [];
      ...
      if (summary) {
        if (facetSink) facetSink.push(summary);

        mdSection += `Facet coverage: **${summary.coveredFacets}/${summary.totalFacets}** facets covered.\n`;
        mdSection += `Automation mix: **${formatAutomationMix(summary.automationCounts)}**.\n`;

        if (summary.missingFacetRules.length) {
          mdSection += `Rules missing facet mapping for this SC: ${summary.missingFacetRules.join(', ')}\n`;
        }

        if (summary.unknownFacetIds && summary.unknownFacetIds.length) {
          mdSection += `Unknown facet ids referenced by rules: ${summary.unknownFacetIds.join(', ')}\n`;
        }

        mdSection += `\n| Facet | Automation | Covered by |\n|---|---|---|\n`;
        for (const f of summary.facets) {
          const coveredBy = f.coveredBy.length ? f.coveredBy.join(', ') : '—';
          mdSection += `| ${f.id} | ${f.automation} | ${coveredBy} |\n`;
        }
        ...
        mdSection += `No facet model defined for this SC yet.\n`;
        if (missingFacetRules.length) {
          mdSection += `Rules missing facet mapping for this SC: ${missingFacetRules.join(', ')}\n`;
        }
        mdSection += `\n`;
      }
      ...
      for (const r of rowsForSc) {
        ...
        const notes = r.error
          ? `ERROR: ${escapeMdHtml(String(r.error)).replaceAll('|', '\\|')}`
          : '';
        const facetIds = getFacetIdsForScFromRow(r, sc);
        const facetText = facetIds.length ? facetIds.join(', ') : '';
        const safeTitle = escapeMdHtml(r.title || '').replaceAll('|', '\\|');
        mdSection += `| ${r.ruleId || ''} | ${r.type || ''} | ${safeTitle} | ${r.file} | ${facetText} | ${notes} |\n`;
      }
      mdSection += `\n`;
    }
