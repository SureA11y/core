 ...
 * version" note), cross-checked against the reference engine's own `wcag21a`/`wcag21aa`/`wcag22aa`-tagged
 * rules in `node_modules/the reference engine/its source` (e.g. `autocomplete-valid` <-> the reference engine's SC-1.3.5 rule
 * carries `wcag21aa` with no `wcag2aa`; `target-size-minimum` <-> the reference engine's SC-2.5.8 rule carries
 * `wcag22aa` alone) -- the reference engine never tags a rule with more than one version prefix, matching the
